<?php
  ...
  include("database.php");
    if(isset ($_GET['action']) && $_GET['action'] == 'edit'){
        $reportvideoID = $_GET['reportvideoID'];

        $sql = "
        SELECT * FROM report_video WHERE reportvideoID = '" . $reportvideoID . "'
      ";
      ...
      $report_video = $result->fetch_assoc();
      // echo '<pre>';
      // print_r($report_video);
      // exit;
      }
      
      if(isset($_POST['buttonEditVideo'])){
        $sql = "
        UPDATE  report_video SET
            status = '" . $_POST['status'] . "'

            WHERE reportvideoID = '" . $_POST['reportvideoID'] . "'
              ";
              ...
              header("location:report-videos.php");
              exit();
      }
?>
<!DOCTYPE html>
              ...
              <div class="panel-heading">
                  ...
                  <div class="panel-body" >
                    ...
                    <form class="" role="form" name="editVideoForm" id="editVideoForm" action="" method="POST" enctype="multipart/form-data">
                      <!--  -->
                    <input type="hidden" id="reportvideoID" name="reportvideoID" value="<?php echo $report_video['reportvideoID']; ?>" > 
                    <div class="row">
                        <div class="col-sm-8">

                      <div class="form-group form-group-default form-group-default-select2 required Form">
                        <label style="font-family: 'Noto Sans Thai', sans-serif;">สถานะ</label>
                        
                        <select id="status" name="status" class="full-width" data-init-plugin="select2" >
                          <?php
                            $IsSeIect1 = '';
                            $IsSeIect2 = '';
                            if ($report_video['status'] == 0){
                              $IsSeIect1 = 'selected';
                            }
                            if ($report_video['status'] == 1){
                              $IsSeIect2 = 'selected';
                            }
                            ?>
                        ...
                        <br>
                        <button type="submit" id="buttonEditVideo" name="buttonEditVideo" class="btn btn-success btn-cons btn-animated from-top fa fa-arrow-down btn-rounded but_save">
